...
// ** Store & Actions
import { getAllData, getData, updatePortfolioItem } from '../store'
...
const PortfolioList = () => {
  ...
  // usual id/desc, matching PortfolioItem::paginate()'s default on the backend.
  ...
  const handleReorder = async (oldIndex, newIndex) => {
    ...
    for (const change of changes) {
      await dispatch(updatePortfolioItem({ id: change.id, sort_order: change.sort_order }))
    }
